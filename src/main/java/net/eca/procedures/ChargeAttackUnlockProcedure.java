...
public class ChargeAttackUnlockProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (((LivingEntity) entity).getAttribute(EpicCoreApiModAttributes.SKILLPOINTS.get()).getValue() >= 1) {
			((LivingEntity) entity).getAttribute(EpicCoreApiModAttributes.SKILLPOINTS.get())
					.addPermanentModifier((new AttributeModifier(UUID.fromString("8617d2da-d647-48f0-a465-cbd5590e7fac"), "skill_points_reduce", (-1), AttributeModifier.Operation.ADDITION)));
			...
			entity.getPersistentData().putBoolean("charge_attack", true);
		}
	}
}
